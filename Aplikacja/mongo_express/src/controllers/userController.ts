...
export const registerUser = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (existingUser) {
      res.status(400).json({ message: "Użytkownik z tym adresem e-mail już istnieje." });
      return;
    }
    ...
    const newUser = new User({
      ...
      isActive: false,  // Konto jest domyślnie nieaktywne
    });
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail",  // Możesz wybrać inny serwis
      auth: {
        user: "your-email@gmail.com",  // Twój e-mail
        pass: "your-email-password",  // Twoje hasło (lub hasło aplikacyjne)
      },
    });
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        console.log(error);
        return res.status(500).json({ message: "Błąd podczas wysyłania wiadomości e-mail." });
      }
      res.status(201).json({ message: "Użytkownik zarejestrowany. Sprawdź swoją skrzynkę pocztową, aby aktywować konto." });
    });
